...
class DestructibleWorldDirector:
    """Small self-contained destruction layer for the two combat arenas."""
    ...
    def _attach(self, mode) -> None:
        ...
        setattr(mode, "nav_revision", int(getattr(mode, "nav_revision", 0)))
        game_id = str(getattr(mode, "game_id", ""))
        specs = self._specs(game_id)
        for index, (pos, size, kind) in enumerate(specs):
            position = Vec3(*pos)
            full = Vec3(*size)
            half = full * 0.5
            base = (0.12, 0.09, 0.055, 1.0) if kind == "crate" else (0.08, 0.09, 0.105, 1.0)
            node = make_box(
                f"destructible-{kind}-{index}",
                tuple(full),
                base,
                mode.world_root,
                tuple(position),
    ...
    def _destroy_prop(self, mode, prop: DestructibleProp) -> None:
        ...
        for index in range(6):
            try:
                ...
                # Debris is short-lived via a Panda interval-free delayed task.
                mode.base.taskMgr.doMethodLater(
                    1.8 + self.rng.random() * 0.8,
                    lambda task, n=debris: (n.removeNode() if not n.isEmpty() else None) or task.done,
    ...
    def _nearest_hit(self, origin: Vec3, direction: Vec3, max_distance: float):
        ...
        if best_prop is None:
            return None, None
        return best_prop, origin + direction * best_t
