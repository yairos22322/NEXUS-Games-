...
class PerkDirector:
    ...
    def _attach(self, mode) -> None:
        ...
        # Runtime weapon perks are read by WeaponLoadoutDirector on every switch.
    ...
    def _open_offer(self, mode) -> None:
        ...
        DirectLabel(
            parent=self.panel,
            text="CHOOSE ONE UPGRADE FOR THIS RUN",
        ...
        for index, perk in enumerate(self.offer):
            ...
            card = DirectFrame(
                parent=self.panel,
                frameColor=(0.035, 0.055, 0.085, 0.94),
                frameSize=(-0.38, 0.38, -0.30, 0.30),
                pos=(x, 0, 0.02),
            )
            DirectFrame(
                parent=card,
                frameColor=accents[index],
                frameSize=(-0.38, 0.38, 0.275, 0.30),
            )
            DirectLabel(
                parent=card,
                text=keys[index],
                text_fg=accents[index],
                text_scale=0.065,
                frameColor=(0,0,0,0),
                pos=(0,0,0.18),
            )
    ...
    def _pool_for(self, mode) -> List[Perk]:
        ...
        if game_id == "neon_ops":
            return [
                ...
                Perk("EXTENDED MAGS", "+6 rounds to every magazine", lambda m: setattr(m, "weapon_mag_bonus", int(getattr(m,"weapon_mag_bonus",0))+6)),
